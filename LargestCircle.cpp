...
bool complete_quads(vector<node> q1, node centre, int radius, vector<string> grid)
{
    ...
    for(node n : q1) {
        ...
        if(!check_free(test1, grid) || !pass_through(centre, radius, test1)) {
            return false;
        }
        node test2(centre.x - (n.x + 1 - centre.x), centre.y + 1 - (n.y - centre.y));
        if(!check_free(test2, grid) || !pass_through(centre, radius, test2)) {
            return false;
        }
        node test3(n.x, centre.y + 1 - (n.y - centre.y));
        if(!check_free(test3, grid)|| !pass_through(centre, radius, test3)) {
            return false;
        }
    }
    return true;
